...
export default function SnapshotsPage() {
  ...
  const handleDownload = () => exportToPNG('snapshot-content', `${type}-snapshot-${date}`);
  const handleShare = () => shareSnapshot('snapshot-content', `${SNAPSHOT_TYPES.find((t) => t.key === type)?.label} - ${date}`);
      ...
      <div id="snapshot-content" className="bg-white rounded-2xl border border-gray-200 p-6 max-w-2xl">
        ...
        <div className="flex items-center justify-between mb-6 pb-4 border-b border-gray-100">
          ...
          <div className="text-right">
            <p className="text-sm font-bold text-gray-700">{new Date(date).toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</p>
                ...
                <div className="bg-gray-50 rounded-xl p-4 flex items-center justify-between">
                  <div>
                    ...
                    <p className={clsx("text-4xl font-black mt-1",
                      data.summary.totalGainLoss > 0 ? "text-green-600" : data.summary.totalGainLoss < 0 ? "text-red-600" : "text-gray-600"
                    )}>
                      ...
                      {data.lineBreakdown.map((lb: any) => (
                        <div key={lb.line.id} className="flex items-center justify-between bg-gray-50 rounded-xl px-4 py-3">
                          ...
                          <div className="flex items-center gap-4 text-sm">
                            ...
                            <span className="font-bold text-gray-800">A: {fmt(lb.summary.totalActualOutput, 0)}</span>
            ...
            {type === 'hourly' && data.hourly && (
              <div className="space-y-3">
                <div className="bg-gray-50 rounded-xl p-4 flex justify-between items-center mb-4">
                  <p className="text-sm text-gray-500">Day Total Gain/Loss</p>
                ...
                {data.hourly.map((h: any) => (
                  <div key={h.label} className="flex items-center justify-between bg-gray-50 rounded-xl px-4 py-3">
                    ...
                    <span className="font-bold text-gray-800 text-sm">A: {fmt(h.summary.totalActualOutput, 0)}</span>
                    <span className={clsx("font-bold text-sm", Number(h.summary.totalActualOutput) >= Number(h.summary.totalTargetOutput) ? "text-green-600" : "text-red-600")}>
